<template>
    <Popover class="w-fit relative py-4" :class="componentName">
        <PopoverButton
        ...
        >
            <div class="flex flex-col">
                ...
                >
                    <span
                        v-if="false"
                        class="h-[65px] w-[65px] overflow-hidden rounded-md"
                    >
                        <img
                            class="h-full w-full"
    ...
    setup(props) {
        ...
        const calculateBlockMenuPosition = () => {
            ...
            const parentElementOffsetHeight =
                selectorButtonRef?.value?.el?.parentElement?.parentElement
                    ?.offsetHeight ?? 1
            const selectorElementOffsetTop =
                selectorButtonRef?.value?.el?.parentElement?.offsetTop ?? 0

            blockMenuPositionYAxis.value =
                selectorElementOffsetTop - parentElementOffsetTop >
                parentElementOffsetHeight / 2
                    ? 'bottom-4'
                    : 'top-4'
        }
